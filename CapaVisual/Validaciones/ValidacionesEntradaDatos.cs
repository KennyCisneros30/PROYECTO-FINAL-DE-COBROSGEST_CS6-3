...
namespace CapaVisual.Validaciones
{
    internal class ValidacionesEntradaDatos
    {
        ...
        {
            // Permitir letras, números y el carácter '-'
            if ((char.IsLetter(e.KeyChar) || char.IsNumber(e.KeyChar) || e.KeyChar == '-') && txt.Text.Length < 8)
            {
                e.Handled = false;
                return true; // Indica que se permitió el ingreso de números, letras y '-'.
            }
            else if (char.IsControl(e.KeyChar))
            {
                e.Handled = false;
                return true; // Permitir teclas de control como Backspace.
            }
            else
            {
                e.Handled = true;
